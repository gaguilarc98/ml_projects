...
#Chapter 1. Classification and regression methods with decission trees and ensemble methods.
#Chapter 2. Bias-variance trade-off
#Chapter 3. Hyperparameter tuning, bagging and random forests
#Chapter 4. Boosting.
#They are easy to explain, they capture non-linear relationships, they do not need much
...
#SELECT A MODEL
df_model <- decision_tree() %>% 
  set_engine("rpart") %>% 
  set_mode("classification")

df_model %>% 
  fit(formula = issetosa~Sepal.Length+Sepal.Width, data=df)
...
df_fit <- df_model %>% 
  fit(issetosa~Sepal.Length+Sepal.Width, data=df_train)

predict(df_fit, new_data=df_test)
df_eval <- df_test %>% 
  select(issetosa) %>% 
  bind_cols(predict(df_fit, new_data=df_test))
df_eval
spec(df_eval, truth = issetosa, .pred_class)
