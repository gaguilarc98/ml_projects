...
#tune() and dials() for fine-tuning models
...
#____LINEAR REGRESSION WITH TIDYMODELS____#
...
lm_model <- linear_reg() %>% #This is aprsnip object
  set_engine('lm') %>% 
  set_mode('regression') 
lm_fit <- lm_model %>% 
  fit(hwy~cty, data=mpg_training) #Formulas to assign roles to columns
tidy(lm_fit)
#glance(lm_fit)
hwy_pred <- lm_fit %>% 
  predict(new_data = mpg_testing)

#Predict returns a tibble, keeps rows in the same order as new_data
#gives the name .pred to the prediction column
...
# 2. Fits models to the training data
# 3. Calculates metrics and predictions
# 4. Returns object with all the results
...
####____ASSESING MODEL'S PERFORMANCE____#####
...
#YOU CAN USE SUMMARY ON confussion matrix as well to get most metrics
summary(df_conf_mat)
