...
for (j in 1:length(arch)) {
  ...
  for (i in 1:1) {
    ...
    bdcSim <- bdcRDS %>% 
      ...
      mutate(previusLast = sum(previus)) %>% 
      ungroup() %>% 
      group_by(CALIFICACION,califPot) %>% 
      summarise(across(everything(),~sum(.x)),
                nops=n(),
                previusLast=max(previusLast)) %>% 
      ungroup() %>% 
      mutate(NSim = i)
...
Prev <- bdcRDS %>% 
  ...
  glimpse()
####____MAKE SURE OF PREVISION____####
arch <- c("avg12","avg12_sens_ab","avg12_sens_ba","P5P95","P10P90","P25P75")
for (i in 1:length(arch)) {
  dpprev <- fread(paste0("D:/!bso/transMat/prev",mes,"/dpprev_",arch[i],".csv"))
  ##ADDING NUMBER OF ITERATION
  dpsim <- dpprev %>% 
    mutate(pos=paste0(CALIFICACION,dplyr::lag(CALIFICACION,1))) %>% 
    mutate(dummy = ifelse(pos=="AF",1,0)) %>% 
    mutate(dummy = cumsum(dummy)) %>% 
    select(-pos)
  write.csv(dpsim,paste0("D:/!bso/transMat/prev",mes,"/dpprev_",arch[i],".csv"),row.names = F)  
}
