...
dp <- paste0('D:/!bso/transMat/prevAbr2023/dpprev_',
...
for (i in 1:length(dp)) {
  ...
  if(i==1){
    ...
    plot_dens <- ggplot(dens,aes(x=difPrev/1e6))+
      ...
      geom_vline(data=quant,aes(xintercept=Value,color=Measure),linewidth=1)+
    ...
    ggsave("D:/!bso/transMat/prevAbr2023/dens_avg12.png",plot_dens,width = 9,height = 6,units = "in")
  }
  densList[[i]] <- dens
}
...
ggplot(dens_all,aes(x=difPrev/1e6, fill = tipo))+
  ...
  geom_vline(data=mean_prev, aes(xintercept = difPrev/1e6),color=paleta(8)[-c(1:2)],linewidth=1,
             show.legend = F)+
  ...
  theme(plot.title = element_text(size = 10,face="bold"),
        legend.position = 'bottom')
ggsave('D:/!bso/transMat/prevAbr23/dens_all_Abr23.png',width = 9,height = 6,units = "in")

####____PARA COMITE____####
dp <- paste0('D:/!bso/transMat/prevAbr2023/dpprev_',
             c('avg12','avg12_sens_ab','avg12_sens_ba','P25P75'),'.csv')
tip <- c('Promedio','A-B +0.05%', 'B-A +5%','Perc 25 y Perc 75')
densList <- list()
for (i in 1:length(dp)) {
  dpprev_pct <- read.csv(dp[i])
  ...
  dens <- dpprev_pct %>% 
    select(-CALIFICACION,-califPot) %>% 
    group_by(dummy) %>% #Agrupo por nro de simulacion realizada
    summarise_all(sum) %>% 
    mutate(tipo = tip[i])
  if(i==1){
    quant <- dens %>% 
...
ggplot(dens_all,aes(x=difPrev/1e6, fill = tipo))+
  geom_density(alpha=0.55)+
  geom_vline(data=mean_prev, aes(xintercept = difPrev/1e6),color=paleta(8)[-c(1:2,7,8)],linewidth=1,
             show.legend = F)+
  scale_fill_manual(values = paleta(8)[-c(1:2,7,8)])+
  # geom_label_repel(data=mean_prev,aes(x=previusNew/1e6,y=6e-1,label=lab),size=3.5,
  #                  show.legend = F)
  annotate(geom="label_repel",x=mean_prev$difPrev/1e6,y=6e-1,
           color = paleta(8)[-c(1:2,7,8)],
           label=mean_prev$labdif,size=3.5)+
  scale_x_continuous(breaks = seq(10,50,5),labels = scales::comma)+
  labs(x="Diferencia de Previsión (MM USD)", y="Densidad", fill="Matriz de transición")+
  theme_light()+ 
  theme(plot.title = element_text(size = 10,face="bold"),
        legend.position = 'bottom')
ggsave('D:/!bso/transMat/prevAbr2023/dens_all_Abr23_Comite.png',width = 9,height = 6,units = "in")
