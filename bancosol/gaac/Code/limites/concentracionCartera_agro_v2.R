...
################################################################################
lim<-read.xlsx('D:/!bso/limites/limitesagro.xlsx')
gc()
cc_list <- list.files('D:/!bso/limites/rdsFCST_m_agro')
cc_list_2<-list()

for(i in 1:length(cc_list)){
  df<-readRDS(paste0('D:/!bso/limites/rdsFCST_m_agro/',
                     cc_list[[i]]))
  cc_list_2[[i]]<-df
}
################################################################################
...
allExp_2 <- readRDS('D:/!bso/limites/proyagro.rds')
...
for(i in 1:length(unique(allExp_2$agro))) {
  ...
  ggsave(paste0('D:/!bso/limites/png/agro_pct_',unique(allExp_2$agro)[[i]],
