...
for(i in 1:length(short_list)){
  ...
  lim<-bdcLim %>% 
    select(monDate, saldous, MONTO, MONEDA) %>%
    mutate(montous = ifelse(MONEDA == 0, as.numeric(MONTO)/6.86, MONTO),
           rangom = ifelse(montous > 20000, '2. mayor a 20k', '1. menor a 20k')) %>% 
...
write_rds(base, 'C:/!bso/concentracion/baseLimites_rangom.rds')

# base<-read.csv('C:/!bso/concentracion/baseLimites_rangom.csv')
################################################################################
#Tabla de Estadisticos

base<-readRDS('C:/!bso/concentracion/output/baseLimites_rangom.rds')

tabla_resumen<-base %>% 
  ungroup() %>% 
  dplyr::filter(rangom!= is.na(rangom)) %>% 
  select(rangom, monDate, carteraBso, pct_saldo) %>% 
  group_by(rangom) %>% 
...
#Pronosticos
 baseFcast<-base %>% 
  dplyr::filter(!is.na(rangom)) %>% 
  dplyr::filter(monDate>'feb. 2015') %>% 
  select(monDate, rangom, carteraBso) %>% 
  dplyr::rename(saldo=carteraBso) %>% 
  # mutate(rangom=str_replace(rangom, ' ', '_')) %>%
  group_by(rangom) %>% 
  arrange(monDate) %>% 
  mutate(delta=(saldo-dplyr::lag(saldo))/dplyr::lag(saldo)) %>% 
  dplyr::filter(!is.na(delta)) %>% 
  #pivot_wider(names_from = rangom, values_from = c(saldo, delta)) %>%
  pivot_longer(!c(monDate, rangom)) %>% 
  arrange(rangom, monDate)
  

for(i in 1:length(unique(baseFcast$rangom))) {
  for(k in 1:length(unique(baseFcast$name))) {
    tryCatch({
      print(paste0(i, ', ', k))
      
      gph <- baseFcast %>% 
        dplyr::filter(rangom == unique(baseFcast$rangom)[i] & name == unique(baseFcast$name)[k]) %>% 
        ungroup()
      ...
      exp <- gph %>%
        ...
        mutate(D = fit$arma[7]) %>%
        mutate(Q = fit$arma[4]) %>%
        fill(rangom, name, .direction = 'updown') %>% 
        relocate(date) %>% 
        mutate(monDate = as.yearmon(date))
      
      write_rds(exp, paste0("C:/!bso/concentracion/rdsFCST_m_rangom/m_", unique(baseFcast$rangom)[i], '_',
...
################################################################################
lim<-read.xlsx('D:/!bso/limites/limitesrangom.xlsx')
gc()
cc_list <- list.files('D:/!bso/limites/rdsFCST_m_rangom')
cc_list_2<-list()

for(i in 1:length(cc_list)){
  df<-readRDS(paste0('D:/!bso/limites/rdsFCST_m_rangom/',
                     cc_list[[i]]))
  cc_list_2[[i]]<-df
}
...
allExp_2<-bind_rows(cc_list_2) %>% 
  ...
  select(monDate, rangom, value, name, starts_with('Hi ') ) %>%
  dplyr::filter(name=='saldo') %>% 
  dplyr::filter(month(monDate)> 3 & year(monDate)==2023) %>% 
  left_join(lim, by='rangom') %>% 
  select(-name) %>% 
  dplyr::rename(saldo=value) %>%
  # mutate(LimiteInicial=ifelse(rangom=='Cochabamba', 25, LimiteInicial)) %>% 
  # mutate(LimiteFinal=ifelse(rangom=='Cochabamba', 26, LimiteFinal)) %>% 
  pivot_longer(!c(monDate, rangom, LimiteInicial, LimiteFinal)) %>% 
  arrange(monDate, name, rangom) %>% 
  group_by(monDate, name) %>% 
  mutate(carteraTotal=sum(value)) %>% 
  ungroup() %>% 
  group_by(monDate, rangom) %>% 
  arrange(monDate, rangom, desc(name)) %>% 
  ...
  arrange(rangom, monDate, desc(name))

write_rds(allExp_2, 'C:/!bso/concentracion/proyrangom.rds')

allExp_2 <- readRDS('D:/!bso/limites/proyrangom.rds')
i <- 3
for(i in 1:length(unique(allExp_2$rangom))) {
...
all_gph <- bind_rows(cc_list_2) %>% 
  ...
  mutate(LimiteInicial=ifelse(rangom=='Cochabamba', 25, LimiteInicial)) %>% 
  mutate(LimiteFinal=ifelse(rangom=='Cochabamba', 26, LimiteFinal)) %>% 
  dplyr::filter(rangom==unique(allExp_2$rangom)[[i]])
...
gph <- all_gph %>% 
  select(rangom, monDate, value,  `Lo 99`,  `Lo 95`,  `Hi 99`,  `Hi 95`) %>% 
...
ggsave(paste0('D:/!bso/limites/png/rangom_',unique(allExp_2$rangom)[[i]],
...
for(i in 1:length(unique(allExp_2$rangom))) {
  ...
  gph <- all_gph %>% 
    select(rangom, monDate, value,  `Lo 99`,  `Lo 95`,  `Hi 99`, 
...
lim2<-mean(gph$LimiteFinal)
  ...
  ggsave(paste0('D:/!bso/limites/png/rangom_pct_',unique(allExp_2$rangom)[[i]],
                '.png'),width = 9,height = 4,units = "in")

}
################################################################################
#export ppt
allexp_ppt <- readRDS('C:/!bso/concentracion/proyrangom.rds') %>%  
  select(monDate, rangom, LimiteInicial, LimiteFinal, pct, value, total_Flag_Inicial,
         total_Flag_Final, first_flag_inicial) %>% 
  #dplyr::filter(monDate=='dic. 2023') %>% 
  dplyr::filter(first_flag_inicial=='TRUE')

write.xlsx(allexp_ppt, 'C:/!bso/concentracion/output/tablaRngo.xlsx')
