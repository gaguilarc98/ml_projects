...
################################################################################
lim<-read.xlsx('D:/!bso/limites/limitesreprog.xlsx')
gc()
cc_list <- list.files('D:/!bso/limites/rdsFCST_m_reprog')
cc_list_2<-list()

for(i in 1:length(cc_list)){
  df<-readRDS(paste0('D:/!bso/limites/rdsFCST_m_reprog/',
                     cc_list[[i]]))
  cc_list_2[[i]]<-df
}
################################################################################
...
allExp_2 <- readRDS('D:/!bso/limites/proyreprog.rds')
...
for(i in 1:length(unique(allExp_2$reprog))) {
  ...
  ggsave(paste0('D:/!bso/limites/png/reprog_pct_',unique(allExp_2$reprog)[[i]],
