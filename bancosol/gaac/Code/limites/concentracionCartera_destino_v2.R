...
for(i in 1:length(short_list)){
  ...
  lim<-bdcLim %>% 
    select(monDate, saldous, AGENCIA, labGrupoC) %>%
    mutate(destino = case_when(labGrupoC== 'A. Agricola' | 
                               ...
                               labGrupoC == 'P. Serv. Doméstico' |  
                                 labGrupoC ==  'Z. Jubilados, Est. y AC'~'Otros',
                                      ...
                                      TRUE ~ labGrupoC)) %>% 
    mutate(destino = ifelse(is.na(destino),'Otros', destino)) %>%
    group_by(monDate, destino) %>% 
...
write_rds(base, 'C:/!bso/concentracion/output/baseLimites_destino.rds')

base<-readRDS('C:/!bso/concentracion/output/baseLimites_destino.rds') 
################################################################################
#Tabla de Estadísticos

tabla_resumen<-base %>% 
  ungroup() %>% 
  dplyr::filter(destino!= is.na(destino)) %>% 
  select(destino, monDate, carteraBso, pct_saldo) %>% 
  group_by(destino) %>% 
...
#Pronosticos
baseFcast<-base %>% 
  dplyr::filter(!is.na(destino)) %>% 
  dplyr::filter(monDate>'feb. 2015') %>% 
  select(monDate, destino, carteraBso) %>% 
  dplyr::rename(saldo=carteraBso) %>% 
  mutate(destino=str_replace(destino, ' ', '_')) %>%
  group_by(destino) %>% 
  arrange(monDate) %>% 
  ...
  pivot_longer(!c(monDate, destino)) %>% 
  arrange(destino, monDate)

for(i in 1:length(unique(baseFcast$destino))) {
  for(k in 1:length(unique(baseFcast$name))) {
    tryCatch({
      print(paste0(i, ', ', k))
      
      gph <- baseFcast %>% 
        dplyr::filter(destino == unique(baseFcast$destino)[i] & name == unique(baseFcast$name)[k]) %>% 
        ungroup()
      ...
      exp <- gph %>%
        ...
        mutate(D = fit$arma[7]) %>%
        mutate(Q = fit$arma[4]) %>%
        fill(destino, name, .direction = 'updown') %>% 
        relocate(date) %>% 
        mutate(monDate = as.yearmon(date))
      
      write_rds(exp, paste0("C:/!bso/concentracion/output/rdsFCST_m_destino/m_", unique(baseFcast$destino)[i], '_',
...
################################################################################
################################################################################
lim<-read.xlsx('D:/!bso/limites/limitesdestino.xlsx')
gc()
cc_list <- list.files('D:/!bso/limites/rdsFCST_m_destino')
cc_list_2<-list()

for(i in 1:length(cc_list)){
  df<-readRDS(paste0('D:/!bso/limites/rdsFCST_m_destino/',
                     cc_list[[i]]))
  cc_list_2[[i]]<-df
}
...
allExp_2<-bind_rows(cc_list_2) %>% 
  # select(monDate, rangom, value, name,  `Hi 90`,  `Hi 91`,  `Hi 92`,  `Hi 93`,
  #        `Hi 94`, `Hi 95`,  `Hi 96`,  `Hi 97`, `Hi 98`,`Hi 99`, `Hi 40` ) %>% 
  select(monDate, destino, value, name, starts_with('Hi ') ) %>%
  dplyr::filter(name=='saldo') %>% 
  dplyr::filter(month(monDate)> 3 & year(monDate)==2023) %>% 
  left_join(lim, by='destino') %>% 
  select(-name) %>% 
  dplyr::rename(saldo=value) %>%
  mutate(LimiteInicial=ifelse(destino=='I._Hoteles', 7.8, LimiteInicial)) %>% 
  mutate(LimiteFinal=ifelse(destino=='I._Hoteles', 8, LimiteFinal)) %>% 
  pivot_longer(!c(monDate, destino, LimiteInicial, LimiteFinal)) %>% 
  arrange(monDate, name, destino) %>% 
  group_by(monDate, name) %>% 
  mutate(carteraTotal=sum(value)) %>% 
  ungroup() %>% 
  group_by(monDate, destino) %>% 
  arrange(monDate, destino, desc(name)) %>% 
  ...
  arrange(destino, monDate, desc(name))

write_rds(allExp_2, 'C:/!bso/concentracion/proydestino.rds')

allExp_2 <- readRDS('D:/!bso/limites/proydestino.rds')

for(i in 1:length(unique(allExp_2$destino))) {
  
  all_gph <- bind_rows(cc_list_2) %>% 
    # select(monDate, rangom, value, name, `Hi 99`) %>% 
    dplyr::filter(name=='saldo') %>% 
    left_join(lim, by='destino') %>% 
    mutate(LimiteInicial=ifelse(destino=='Cochabamba', 25, LimiteInicial)) %>% 
    mutate(LimiteFinal=ifelse(destino=='Cochabamba', 26, LimiteFinal)) %>% 
    dplyr::filter(destino==unique(allExp_2$destino)[[i]])
  ...
  gph <- all_gph %>% 
    select(destino, monDate, value,  `Lo 99`,  `Lo 95`,  `Hi 99`,  `Hi 95`) %>% 
  ...
  ggsave(paste0('C:/!bso/concentracion/png/destino_',unique(allExp_2$destino)[[i]],
                '.png'))
...
for(i in 1:length(unique(allExp_2$destino))) {
  ...
  gph <- all_gph %>% 
    select(destino, monDate, value,  `Lo 99`,  `Lo 95`,  `Hi 99`, 
  ...
  ggsave(paste0('D:/!bso/limites/png/destino_pct_',unique(allExp_2$destino)[[i]],
                '.png'),width = 9,height = 4,units = "in")
}

################################################################################
#exportar ppt
allexp_ppt <- readRDS('C:/!bso/concentracion/proydestino.rds') %>%  
  select(monDate, destino, LimiteInicial, LimiteFinal, pct, value, total_Flag_Inicial,
         total_Flag_Final, first_flag_inicial) %>% 
  #dplyr::filter(monDate=='dic. 2023') %>% 
  dplyr::filter(first_flag_inicial=='TRUE')

write.xlsx(allexp_ppt, 'C:/!bso/concentracion/output/tablaDest.xlsx')
