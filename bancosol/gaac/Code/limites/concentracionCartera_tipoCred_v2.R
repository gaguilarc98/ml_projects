...
write_rds(base, 'C:/!bso/concentracion/output/baseLimites_tipoCred.rds')
...
for(i in 1:length(unique(baseFcast$tipoCred))) {
  for(k in 1:length(unique(baseFcast$name))) {
    tryCatch({
      ...
      write_rds(exp, paste0("C:/!bso/concentracion/output/rdsFCST_m_tipoCred/m_", unique(baseFcast$tipoCred)[i], '_',
...
################################################################################
lim<-read.xlsx('D:/!bso/limites/limitesTipoCred.xlsx') %>% 
  mutate(tipoCred=ifelse(tipoCred=="Microcredito","Micro",tipoCred)) %>% 
  mutate(tipoCred=ifelse(tipoCred=="Pyme","PyMe",tipoCred))
           
gc()
cc_list <- list.files('D:/!bso/limites/rdsFCST_m_tipoCred')
cc_list_2<-list()

for(i in 1:length(cc_list)){
  df<-readRDS(paste0('D:/!bso/limites/rdsFCST_m_tipoCred/',
                     cc_list[[i]]))
  cc_list_2[[i]]<-df
}
...
allExp_2 <- readRDS('D:/!bso/limites/proytipoCred.rds')
i <- 3
...
ggsave(paste0('D:/!bso/limites/png/tipoCred_',unique(allExp_2$tipoCred)[[i]],
              '.png'))
...
lim2<-mean(gph$LimiteFinal)
  ...
  ggsave(paste0('D:/!bso/limites/png/tipoCred_pct_',unique(allExp_2$tipoCred)[[i]],
