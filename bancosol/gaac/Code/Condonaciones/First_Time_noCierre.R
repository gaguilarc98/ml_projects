...
moraIntraMes <- readRDS(paste0("D:/!bso/firstTimes/moraIntraMes_Ene2018Jul2023.rds")) %>% 
...
measuresMoraIM <- moraIntraMes %>% 
  ...
  summarise(meanMoraIM = mean(maximaDPD_2),
            mesesDesde18 = n(),
            IntDPD1 = sum(tuvoDPD_1)/mesesDesde18,
            IntDPD5 = sum(tuvoDPD_5)/mesesDesde18,
            IntDPD10 = sum(tuvoDPD_10)/mesesDesde18,
            IntDPD15 = sum(tuvoDPD_15)/mesesDesde18,
            IntDPD20 = sum(tuvoDPD_20)/mesesDesde18,
            IntDPD25 = sum(tuvoDPD_25)/mesesDesde18) %>% 
  ungroup()
dpd1 <- moraIntraMes %>% 
  dplyr::filter(tuvoDPD_1>0) %>% 
  group_by(CTACLIENTE, OPERACION) %>% 
  summarise(myFirst_DPD1 = min(monDate)) %>% 
  ungroup()
system.time(
dpd5 <- moraIntraMes %>% 
  dplyr::filter(tuvoDPD_5>0) %>% 
  group_by(CTACLIENTE, OPERACION) %>% 
  summarise(myFirst_DPD5 = min(monDate)) %>% 
  ungroup()
)
dpd10 <- moraIntraMes %>% 
  dplyr::filter(tuvoDPD_10>0) %>% 
  group_by(CTACLIENTE, OPERACION) %>% 
  summarise(myFirst_DPD10 = min(monDate)) %>% 
  ungroup()
dpd15 <- moraIntraMes %>% 
  dplyr::filter(tuvoDPD_15>0) %>% 
  group_by(CTACLIENTE, OPERACION) %>% 
  summarise(myFirst_DPD15 = min(monDate)) %>% 
  ungroup()
dpd20 <- moraIntraMes %>% 
  dplyr::filter(tuvoDPD_20>0) %>% 
  group_by(CTACLIENTE, OPERACION) %>% 
  summarise(myFirst_DPD20 = min(monDate)) %>% 
  ungroup()
dpd25 <- moraIntraMes %>% 
  dplyr::filter(tuvoDPD_25>0) %>% 
  group_by(CTACLIENTE, OPERACION) %>% 
...
sapply(moraIntraMes, function(x){sum(is.na(x))})
measuresMoraIM <- moraIntraMes %>% 
