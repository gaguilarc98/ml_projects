...
####____PROCESS PAGOS FULL RAW____####
P2full <- Pagos %>% 
...
####____ADDING A MONTH TO PAGOS FULL LAST PROCESSED____####
...
P2uh <- P2full %>%
  ...
  dplyr::filter(myPago >= lastmonth12) %>% #Se actualiza para conservar los últimos 12 meses
...
write_rds(bdcEvol,paste0('D:/!bso/mph/bdcEvol',shortmonth12,shortmonth,'.rds'))
...
write.csv(lastCierre,paste0('D:/!bso/mph/Oreports/lastCierrreUR_',shortmonth,'csv'),
          row.names = F)
