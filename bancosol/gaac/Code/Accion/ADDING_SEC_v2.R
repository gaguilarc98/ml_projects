...
MasterKeyList <- readRDS("//VFSNALSRV/RiesgoCrediticioOFN/Bases_Riesgos/MIS_SGNRFAR/analitica/accion/MasterKeyList.rds") %>% 
...
length(which(!stage_wsec$MASCARA_CUENTA %in% dfAux$MASCARA_CUENTA))
i <- 36
for (i in 1:length(mycsv)) {
  tryCatch({
    print(mycsv[[i]])
    stage1 <- fread(paste0('//VFSNALSRV/RiesgoCrediticioOFN/Bases_Riesgos/MIS_SGNRFAR/analitica/deliver_final/deliver_Etapa_1_csv/',
                           mycsv[i],'.csv'),encoding = "UTF-8",sep=",") %>% 
      
      separate_wider_delim(LLAVEPRIMARIA,delim="-",names = c("MASCARA_CUENTA","MASCARA_OPERACION"),too_few = "align_start",too_many = "drop") %>% 
      mutate(LLAVEPRIMARIA = paste(MASCARA_CUENTA,MASCARA_OPERACION,sep="-")) %>% 
    ...
    dfAux <- dfTotal %>% 
      ...
      group_by(CTACLIENTE) %>% 
      summarise(N_OPNOREFIN = length(which(fueRefin==0)), N_OPREFIN = length(which(fueRefin==1))) %>% 
      left_join(MasterKeyList,by=c("CTACLIENTE")) %>% 
      select(MASCARA_CUENTA,N_OPNOREFIN,N_OPREFIN) %>% 
    ...
    fwrite(stage_wsec,paste0('//VFSNALSRV/RiesgoCrediticioOFN/Bases_Riesgos/MIS_SGNRFAR/analitica/deliver_Etapa_1_csv/',
...
for (i in 1:length(mycsv)) {
  tryCatch({
    print(mycsv[[i]])
    stage1 <- fread(paste0('//VFSNALSRV/RiesgoCrediticioOFN/Bases_Riesgos/MIS_SGNRFAR/analitica/deliver_Etapa_1_csv/',
                           mycsv[i],'.csv'),encoding = "UTF-8",sep=",") %>% 
      select(Fecha.de.Corte,LLAVEPRIMARIA,N_OPNOREFIN,N_OPREFIN) %>% 
      mutate(N_OP = N_OPNOREFIN+N_OPREFIN)
    llavesList[[i]] <- stage1
  }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
}
llavesFull <- rbindlist(llavesList)
fwrite(llavesFull,paste0('//VFSNALSRV/RiesgoCrediticioOFN/Bases_Riesgos/MIS_SGNRFAR/analitica/deliver_Etapa_1_csv/keys_sequence.txt'),
       row.names = F,sep=",")
