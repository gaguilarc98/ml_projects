...
for(i in 1:length(year)) {
  for (k in 1:length(mes)) {
    tryCatch({
      ...
      bdcMora <- readRDS(paste0("D:/!bso/girCartera/rds/ec_",mes[k],year[i],".rds")) %>% 
      # bdcMora <- fread(paste0('//VFSNALSRV/RiesgoCrediticioOFN/Bases_Riesgos/05_Base_Cartera_Mensual/BaseCartera', 
      #                     mes[k], year[i],'.txt'), 
      #              encoding = 'Latin-1', fill = T) %>%
        ...
        select(monDate, CTACLIENTE, OPERACION, ctaCont, esFSL, FVEN_PROXPAGO, 
        ...
        mutate(tuvoDPD1 = ifelse(maximaDPD_2>=1, 1, 0)) %>% 
        mutate(tuvoDPD5 = ifelse(maximaDPD_2>=5, 1, 0)) %>% 
        mutate(tuvoDPD10 = ifelse(maximaDPD_2>=10, 1, 0)) %>% 
        mutate(tuvoDPD15 = ifelse(maximaDPD_2>=15, 1, 0)) %>% 
        mutate(tuvoDPD20 = ifelse(maximaDPD_2>=20, 1, 0)) %>% 
        mutate(tuvoDPD25 = ifelse(maximaDPD_2>=25, 1, 0))
...
saveRDS(moraFull, "D:/!bso/accion/moraIntraMes_Ene2018Ago2023.rds")
...
moraFull <- readRDS("D:/!bso/accion/moraIntraMes_Ene2018Sep2023.rds")
moraFull <- moraFull %>% 
  bind_rows(bdcMora)
saveRDS(moraFull, "D:/!bso/accion/moraIntraMes_Ene2018Oct2023.rds")
...
####____EJEMPLOS DE CALCULO DE DPD____####
bdcMora <- readRDS(paste0("D:/!bso/girCartera/rds/ec_",mes[k],year[i],".rds")) %>% 
  # bdcMora <- fread(paste0('//VFSNALSRV/RiesgoCrediticioOFN/Bases_Riesgos/05_Base_Cartera_Mensual/BaseCartera', 
  #                     mes[k], year[i],'.txt'), 
  #              encoding = 'Latin-1', fill = T) %>%
  ...
  # mutate(across(c(FDESEMBOLSO,FFINALIZA,FULT_PAGO,FVEN_ULTPAGO,FVEN_PROXPAGO,FALTACLI),~dmy(.x))) %>% 
  mutate(esFSL = ifelse(MODULO==118 | str_detect(TIPO_OPER, "MIGR"), 1, 0)) %>% 
  select(monDate, CTACLIENTE, OPERACION, ctaCont, esFSL, FDESEMBOLSO, MONTO, SALDO, MONEDA, 
...
MIMExamples <- bdcMora %>% 
  ...
  mutate(conMora = ifelse(DIASMORA==0,1,0)) %>% 
  mutate(conMIM = ifelse(maximaDPD_2>0,1,0)) %>% 
  group_by(ctaCont, conMora, conMIM) %>% 
  dplyr::filter(row_number()==1) %>% 
  ungroup() %>% 
  select(-conMora,-conMIM)

write.xlsx(MIMExamples, "D:/!bso/requests/EjemplosDPD_Oct2023.xlsx")
