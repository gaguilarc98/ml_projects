...
for (i in 1:length(file_list)) {
  ...
  bdc <- readRDS(paste0('D:/!bso/girCartera/rdsGAR/',
    ...
    select(CTACLIENTE, OPERACION, CI, saldous, ESTADO, tipoCred, DIASMORA, 
...
bdcFull <- bdcFull %>% 
  dplyr::filter(CALIFICACION %in% c("A","B","C","D","E","F","S","Z"))
bdcList <- NULL
write_rds(bdcFull, 'D:/!bso/transMat/bdcFull2.rds')
...
bdcTrans <- bdcFull %>% 
  ...
  glimpse()
table(bdcTrans[bdcTrans$monDate == 'oct. 2022',]$trans)
write_rds(bdcTrans, 'D:/!bso/transMat/bdcTrans.rds')
bdcTrans <- readRDS( 'D:/!bso/transMat/bdcTrans.rds')
# #---------------------------------------------------------
# # counting saldo
bdcTrans3 <- bdcFull %>%
  select(OPERACION, CALIFICACION, monDate, saldous, previus) %>%
  mutate(monDate = zoo::as.yearmon(monDate)) %>%
  #select(-dayDate) %>%
  #dplyr::filter(monDate >= 'Jan 2017') %>%
  group_by(OPERACION) %>%
...
# write.csv(bdcTrans, 'D:/!bso/transMat/bdcTrans_wSaldo.csv')
########
...
write.xlsx(tm2022_prob, 'D:/!bso/transMat/tmprob_2022_oct.xlsx')
...
tm2022_ops <- tm %>% 
  ...
  #select(-`NA`) %>% 
  replace_na(list(A = 0, B = 0, C = 0, D=0,E=0, `F` = 0, S = 0, Z = 0))
write.xlsx(tm2022_ops, 'D:/!bso/transMat/tmops_2022_oct.xlsx')
...
tm2022_saldo <- tm_all %>% 
  ...
  #select(-`NA`) %>% 
  replace_na(list(A = 0, B = 0, C = 0, D=0,E=0, `F` = 0, S = 0, Z = 0))
write.xlsx(tm2022_saldo, 'D:/!bso/transMat/tmsaldo_2022_oct.xlsx')
...
tm2022_previ <- tm_all %>% 
  ...
  #select(-`NA`) %>% 
  replace_na(list(A = 0, B = 0, C = 0, D=0,E=0, `F` = 0, S = 0, Z = 0))
write.xlsx(tm2022_previ, 'D:/!bso/transMat/tmprevi_2022_oct.xlsx')
...
tm2022_difPrev <- tm_all %>% 
  ...
  #select(-`NA`) %>% 
  replace_na(list(A = 0, B = 0, C = 0, D=0,E=0, `F` = 0, S = 0, Z = 0))
write.xlsx(tm2022_difPrev, 'D:/!bso/transMat/tmdifprev_2022_oct.xlsx')
