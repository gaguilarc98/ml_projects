...
for(i in 1:length(mes)) {
  for(k in 1:length(years)) {
    tryCatch({
      ...
      #                     mes[i], years[k],'.txt'), 
      #              encoding = 'Latin-1', fill = T,nrows = 10)
      ...
      bdc <- fread(paste0('//VFSNALSRV/RiesgoCrediticioOFN/Bases_Riesgos/05_Base_Cartera_Mensual/BaseCartera', 
        ...
        mutate(fbase = paste0(mes[i], years[k])) %>% 
        mutate(monDate = as.yearmon(paste0(mes[i],'. ',years[k]))) %>% 
      ...
      saveRDS(bdc, paste0('D:/!bso/girCartera/rds/ec_',
                          mes[i], years[k], '.rds'))
      # saveRDS(bdc, paste0('//VFSNALSRV/RiesgoCrediticioOFN/Bases_Riesgos/MIS_SGNRFAR/rdsGAR/ec_',
      #                     mes[i], years[k], '.rds'))
      #bdcList[[i]] <- bdc
...
####____CARTERA POR SECTOR ACTIVIDAD____####
bdcNC <- readRDS('D:/!bso/girCartera/rdsGAR/ec_Nov2022.rds') 
gph <- bdcNC %>% 
  ...
  summarise(saldous=sum(saldous),saldoCart=sum(saldous)/max(saldoTot)*100) %>% 
  ungroup() %>% 
  adorn_totals("row")
table(bdcNC$Sector_Actividad)

####____CARTERA POR SUCURSAL____####
bdcNC <- readRDS('D:/!bso/girCartera/rdsGAR/ec_Ene2023.rds') 
gph <- bdcNC %>% 
  mutate(saldoTot=sum(saldous)) %>%
  # mutate(Sector_Actividad = case_when(labGrupoC== 'A. Agricola' | 
  #                                       labGrupoC == 'B. Caza, Pesca' ~'Agropecuario',
...
####____MORA REPROG SUCURSAL____####

bdcNC <- readRDS('D:/!bso/girCartera/rdsGAR/ec_Nov2022.rds') 
gph <- bdcNC %>% 
  select(saldous,par30,sucursal,saldoReprog,par30Reprog) %>% 
  group_by(sucursal) %>% 
  summarise_all(sum,na.rm=T) %>% 
  mutate(pct=saldous/sum(saldous)*100) %>% 
  adorn_totals('row') %>% 
  mutate(par0Rel = par30/saldous*100) %>% 
  mutate(morareprog = par30Reprog/saldoReprog*100)
  # mutate(Sector_Actividad = case_when(labGrupoC== 'A. Agricola' | 
  #                                       labGrupoC == 'B. Caza, Pesca' ~'Agropecuario',
  #                                     labGrupoC == 'M. Adm. Pública' |  
  #                                       labGrupoC ==  'N. Educación' ~'Asalariados',
...
####SECTORACTUVIDAD####
bdc <- readRDS('C:/!bso/ec_Dic2022.rds')
gph<-bdcNC %>% 
  dplyr::filter(MODULO != 131) %>%
