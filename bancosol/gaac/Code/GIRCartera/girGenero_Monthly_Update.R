...
####____NEW STYLE FROM TABLA GIR____####
tablaGIR <- fread("//VFSNALSRV/RiesgoCrediticioOFN/Bases_Riesgos/MIS_SGNRFAR/analitica/tablaGIR/tablaGIR.csv",
                  encoding="UTF-8",sep=",",fill = T) %>% 
  glimpse()

tabSucursal <- tablaGIR %>% 
  dplyr::filter(Fecha>=as.Date('2022-11-30')) %>% 
