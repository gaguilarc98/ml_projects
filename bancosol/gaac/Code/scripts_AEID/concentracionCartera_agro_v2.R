...
for(i in 1:length(short_list)){
  ...
  lim<-bdcLim %>% 
    select(monDate, saldous, AGENCIA, labGrupoD) %>%
    mutate(agro = ifelse(labGrupoD== 'A. Agricola' | 
                                 labGrupoD == 'B. Caza, Pesca', 'Agropecuario', 'No Agro')) %>% 
    mutate(agro = ifelse(is.na(agro),'Otros', agro)) %>%
    group_by(monDate, agro) %>% 
...
write_rds(base, 'C:/!bso/concentracion/output/baseLimites_agropecuario.rds')
...
tabla_resumen<-base %>% 
  ungroup() %>% 
  dplyr::filter(agro!= is.na(agro)) %>% 
  select(agro, monDate, carteraBso, pct_saldo) %>% 
  group_by(agro) %>% 
...
#Pronósticos
baseFcast<-base %>% 
  dplyr::filter(!is.na(agro)) %>% 
  dplyr::filter(monDate>'feb. 2015') %>% 
  select(monDate, agro, carteraBso) %>% 
  dplyr::rename(saldo=carteraBso) %>% 
  mutate(agro=str_replace(agro, ' ', '_')) %>%
  group_by(agro) %>% 
  arrange(monDate) %>% 
  ...
  pivot_longer(!c(monDate, agro)) %>% 
  arrange(agro, monDate)

for(i in 1:length(unique(baseFcast$agro))) {
  for(k in 1:length(unique(baseFcast$name))) {
    tryCatch({
      print(paste0(i, ', ', k))
      
      gph <- baseFcast %>% 
        dplyr::filter(agro == unique(baseFcast$agro)[i] & name == unique(baseFcast$name)[k]) %>% 
        ungroup()
      ...
      exp <- gph %>%
        ...
        mutate(D = fit$arma[7]) %>%
        mutate(Q = fit$arma[4]) %>%
        fill(agro, name, .direction = 'updown') %>% 
        relocate(date) %>% 
        mutate(monDate = as.yearmon(date))
      
      write_rds(exp, paste0("C:/!bso/concentracion/output/rdsFCST_m_agro/m_", unique(baseFcast$agro)[i], '_',
...
################################################################################
lim<-read.xlsx('C:/!bso/concentracion/limitesagro.xlsx')
gc()
cc_list <- list.files('C:/!bso/concentracion/output/rdsFCST_m_agro')
cc_list_2<-list()

for(i in 1:length(cc_list)){
  df<-readRDS(paste0('C:/!bso/concentracion/output/rdsFCST_m_agro/',
                     cc_list[[i]]))
  cc_list_2[[i]]<-df
}
################################################################################
...
allExp_2<-bind_rows(cc_list_2) %>% 
  ...
  pivot_longer(!c(monDate, agro, LimiteInicial, LimiteFinal)) %>% 
  arrange(monDate, name, agro) %>% 
  group_by(monDate, name) %>% 
  mutate(carteraTotal=sum(value)) %>% 
  ungroup() %>% 
  group_by(monDate, agro) %>% 
  arrange(monDate, agro, desc(name)) %>% 
  ...
  arrange(agro, monDate, desc(name))

write_rds(allExp_2, 'C:/!bso/concentracion/proyagro.rds')
allExp_2 <- readRDS('C:/!bso/concentracion/proyagro.rds')

for(i in 1:length(unique(allExp_2$agro))) {
  
  all_gph <- bind_rows(cc_list_2) %>% 
    # select(monDate, rangom, value, name, `Hi 99`) %>% 
    dplyr::filter(name=='saldo') %>% 
    left_join(lim, by='agro') %>% 
    dplyr::filter(agro==unique(allExp_2$agro)[[i]])
  ...
  gph <- all_gph %>% 
    select(agro, monDate, value,  `Lo 99`,  `Lo 95`,  `Hi 99`,  `Hi 95`) %>% 
  ...
  ggsave(paste0('C:/!bso/concentracion/png/agro_',unique(allExp_2$agro)[[i]],
                '.png'))
...
for(i in 1:length(unique(allExp_2$agro))) {
  ...
  gph <- all_gph %>% 
    select(agro, monDate, value,  `Lo 99`,  `Lo 95`,  `Hi 99`, 
  ...
  ggsave(paste0('C:/!bso/concentracion/png/agro_pct_',unique(allExp_2$agro)[[i]],
                '.png'))
...
################################################################################
#exportar ppt
allexp_ppt <- readRDS('C:/!bso/concentracion/proyagro.rds') %>%  
  select(monDate, agro, LimiteInicial, LimiteFinal, pct, value, total_Flag_Inicial,
