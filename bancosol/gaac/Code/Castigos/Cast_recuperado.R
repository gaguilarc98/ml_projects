...
for (i in 1:(length(myrds)-1)) {
  tryCatch({
    ...
    df1 <- readRDS(paste0('D:/!bso/girCartera/rdsGAR/ec_',myrds[i],'.rds')) %>% 
      ...
      select(OPERACION,CTACLIENTE,saldoCast) %>% 
      rename(susPrev=saldoCast)
    df2 <- readRDS(paste0('D:/!bso/girCartera/rdsGAR/ec_',myrds[k],'.rds')) %>% 
      dplyr::filter(ESTADO=="CASTIGADA") %>% 
      select(OPERACION,CTACLIENTE,saldoCast) %>% 
      rename(sus=saldoCast)
    df3 <- df2 %>% 
      inner_join(df1,by=c("OPERACION","CTACLIENTE")) %>% 
      mutate(saldoCast=susPrev-sus) %>% 
      mutate(mycast = as.yearmon(paste0(substr(myrds[k],1,3),". ",substr(myrds[k],4,7)))) %>% 
      mutate(myPrevcast = as.yearmon(paste0(substr(myrds[i],1,3),". ",substr(myrds[i],4,7))))
    df4 <- df1[!(df1$OPERACION %in% df2$OPERACION),] %>% 
      rename(saldoCast=susPrev) %>% 
      mutate(mycast = as.yearmon(paste0(substr(myrds[k],1,3),". ",substr(myrds[k],4,7)))) %>% 
      mutate(myPrevcast = as.yearmon(paste0(substr(myrds[i],1,3),". ",substr(myrds[i],4,7))))
...
castFull <- bind_rows(castList) %>% 
  arrange(OPERACION,CTACLIENTE,mycast) %>% 
  glimpse()
write_rds(castFull,'D:/!bso/castigos/castFullEne15Ene23.rds')

castFull <- read_rds('D:/!bso/castigos/castFullEne15Ene23.rds')

castRec <- castFull %>% 
  arrange(OPERACION,CTACLIENTE,mycast) %>% 
  mutate(year=year(mycast)) %>% 
  group_by(year) %>% 
  summarise(saldoRec=sum(saldoCast,na.rm=T))
write.xlsx(castFull2,"D:/!bso/castigos/castFull.xlsx")

ggplot(castFull2, aes(x = year, y = saldoRec/1000))+ 
  geom_bar(stat = 'identity', position = 'dodge', fill = paleta(12)[3])+
  geom_label(aes(label = paste0(as.character(format(round(saldoRec/1000), big.mark = ',')),'$')), 
             color = paleta(12)[3], fontface = 'bold', vjust=1)+
  labs(x="",y="Recuperación de Castigos (M de USD)")+
...
ggsave('D:/!bso/castigos/saldoCondRecuperado.png',height = 6,width = 9,units = "in")

castFull2 <- castFull %>%
  mutate(year=year(mycast)) %>% 
  group_by(year) %>% 
  mutate(saldoRec=sum(saldoCast,na.rm=T)) %>% 
  group_by(CTACLIENTE,OPERACION) %>% 
  arrange(myPrevcast) %>% 
  dplyr::filter(row_number()==1) %>% 
  ungroup() %>% 
  group_by(year) %>% 
  summarise(SaldoCast=sum(susPrev,na.rm = T),SaldoRec=max(saldoRec,na.rm = T)) %>% 
  mutate(saldoCum=cumsum(SaldoCast))

castFull2 %>% 
  mutate(SaldoCastmRec = SaldoCast-SaldoRec) %>% 
  mutate(saldoCum2=cumsum(SaldoCastmRec))
...
write_rds(bdcCast,'D:/!bso/castigos/bdcCast.rds')
...
gph <- bdcCast %>% 
  ...
  select(key, year, saldoCast, SaldoBSO) %>% 
...
bdcCastRaw <- bdcCastCancel %>% 
  ...
  mutate(SaldoRaw_Rel = SaldoCastRaw/SaldoBSO*100) %>% 
...
####____**Castigado Recuperado____####
gph <- bdcCastCancel %>% 
  dplyr::filter(SaldoCancel>0) %>% 
  select(key, year, SaldoCancel, SaldoBSO) %>% 
  mutate(saldoCancel_Rel=SaldoCancel/SaldoBSO*100) %>% 
  group_by(year) %>%
  summarise(SaldoCancel = sum(SaldoCancel,na.rm = T), 
...
####____CONSERVANDO 4 INSTANCIAS ____####
bdc4times <- bdcCast %>% 
  dplyr::filter(rowCast==1 | rowprevCast==1 | pos==1 | pos == max(pos)) %>% 
  ungroup() 

write_rds(bdc4times, 'D:/!bso/castigos/castigos4times.rds')
####____COMPOSICION RECUPERADOS____####
...
plt <- agrupar(gph, vstep=year, vgrupo=binCastMonto, vagre=SaldoCancel,pct=5,last = 3)
plt$y <- plt$y %>% 
  mutate(lab = paste0(round(rat),'% (',comma(round(tot/1e3)),' M USD)'))
ggplot(plt$y,aes(x=year,y=rat,fill=ORDEN))+
  geom_bar(stat="identity")+
...
plt$y <- plt$y %>% 
  mutate(ORDEN = as.character(ORDEN)) %>% 
  mutate(ORDEN = ifelse(ORDEN=='Otros',ORDEN,as.character(year-as.integer(ORDEN)))) %>% 
  mutate(ORDEN = ifelse(ORDEN=='Otros',ORDEN,paste0("t - ",ORDEN)))

####____**Por tipo Cred____####
...
ggplot(plt$y,aes(x=year,y=tot,fill=ORDEN))+
  ...
  theme_minimal()+
  theme(legend.position = "bottom")
ggsave('D:/!bso/castigos/castTipoCred.png',width = 9,height = 6,units = "in")
...
####____**Por rangom____####
plt <- agrupar(gph,vstep = year, vgrupo = rangom, vagre = saldoCast, pct=8)
plt$y <- plt$y %>% 
  mutate(lab = paste0(round(rat),'% (',round(tot/1e3,1),' MM USD)'))
ggplot(plt$y,aes(x=year,y=tot,fill=ORDEN))+
  geom_bar(stat="identity")+
  geom_label(aes(label=lab),size=3,color="white",show.legend = F,
            position=position_stack(vjust=0.5))+
  ...
  theme_minimal()+
  theme(legend.position = "bottom")
ggsave('D:/!bso/castigos/castRangom.png',width = 9,height = 6, units = "in")
####____**Por rangos____####
plt <- agrupar(gph,vstep = year, vgrupo = rangos, vagre = saldoCast, pct=5)
plt$y <- plt$y %>% 
  mutate(lab = paste0(round(rat),'% (',round(tot/1e3,1),' MM USD)'))
ggplot(plt$y,aes(x=year,y=tot,fill=ORDEN))+
  geom_bar(stat="identity")+
  ...
  theme_minimal()+
  theme(legend.position = "bottom")
ggsave('D:/!bso/castigos/castRangos.png',width = 9,height = 6, units = "in")
####____**Por Sucursal____####
plt <- agrupar(gph,vstep = year, vgrupo = Sucursal, vagre = saldoCast, pct=8)
plt$y <- plt$y %>% 
  mutate(lab = paste0(round(rat),'% (',round(tot/1e3,1),' MM USD)'))
ggplot(plt$y,aes(x=year,y=tot,fill=ORDEN))+
  geom_bar(stat="identity")+
  ...
  theme_minimal()+
  theme(legend.position = "bottom")
ggsave('D:/!bso/castigos/castSucursal.png',width = 9,height = 6, units = "in")

####____LISTA DE CASTIGOS___####
bdcCast <- readRDS('D:/!bso/castigos/castigos4times.rds')
bdcCast <- readRDS('D:/!bso/castigos/bdcCast.rds')
codAge <- read_excel('D:/!bso/bases/excel/CodAgeSucReg.xlsx')
