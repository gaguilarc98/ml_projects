...
bdcVIP_mar23 <- readRDS('D:/!bso/vipRC/bdcVIP_mar2023.rds')
#===============================================================================
# bdc agosto
bdcLast <- readRDS('D:/!bso/girCartera/rds/ec_Feb2023.rds')
bdcAgo <- bdcLast %>% 
...
bdcAgoVipNoMiss_mar23 <- readRDS('D:/!bso/vipRC/bdcAgoVipNoMiss_mar2023.rds')
bdcAgo_mar23 <- readRDS('D:/!bso/vipRC/bdcAgo_mar2023.rds')
#===============================================================================
# infocred
infoRaw_mar23 <- fread('D:/!bso/califClientes/utf/BSO202302_utf8.txt', encoding = 'UTF-8')
nrowInfo <- nrow(infoRaw)

infoPerf_mar23 <- infoRaw_mar23 %>% 
...
bdcAgoInfo <- bdcAgoVipNoMiss %>% 
  ...
  left_join(infoPerf_mar23, by = 'CTACLIENTE') %>% 
  select(-CI.x, -CI.y)


bdcAgoInfoCL <- bdcAgoInfo
write_rds(bdcAgoInfoCL, 'D:/!bso/vipRC/bdcAgoInfoCL_feb2023_v2.rds')
bdcAgoInfoCL_mar23 <- readRDS('D:/!bso/vipRC/bdcAgoInfoCL_mar2023.rds')
#===============================================================================
# daily delinquency data
dailyMM_mar23 <- readRDS('C:/!bso/vipCartera/dailyMM_mar2023.rds') %>%
  glimpse() %>% 
  ungroup() %>% 
...
bdcAgoInfoCL_dailyVal_mar23 <- bdcAgoInfoCL_mar23 %>% 
  left_join(dailyMM_feb23, by = 'CTACLIENTE') %>% 
...
bus_mar23 <- read.delim('D:/!bso/vipCartera/Clientes excelentes 202303.csv',
                  sep = '|') %>%
# bus_feb23 <- read.xlsx('D:/!bso/vipCartera/Clientes Excelentes Febrero 2023.xlsx',
#                   sep = '|') %>% 
  dplyr::rename(CTACLIENTE = Cuenta) %>% 
  select(CTACLIENTE) %>% 
  distinct() %>% 
  mutate(excelenteNegocios = 1)
length(unique(bus_mar23$CTACLIENTE))

finalVIPdf_mar23 <- bdcAgoInfoCL_dailyVal_mar23 %>% 
  left_join(bus_mar23, by = 'CTACLIENTE') %>% 
...
bdcVIPscore_mar23 <- readRDS('D:/!bso/vipRC/bdcVIPscore_mar2023.rds')
bdcVIPscore_feb23_v2 <- readRDS('D:/!bso/vipRC/bdcVIPscore_feb2023_v2.rds')
#==============================================================================
# DEFINICIÓN DE CLIENTE VIP
bdcVIPScore_t_mar23 <- bdcVIPscore_mar23 %>% 
...
# Table export
bdcVIPScore_mar23 <- readRDS('D:/!bso/vipRC/bdcVIPScore_mar2023.rds')
bdcVIPScore_exp_mar23 <- bdcVIPScore_mar23 %>% 
  mutate(target = ifelse(((pctMaxBin == '5. 0 (sin morosidad)' | pctMaxBin == '1. 90% +') &
                            (pctAcumBin == '1. 0%') &
                            (tldBin == '4. 3240 +' | tldBin == '3. 2160-3240 días' | 
                               tldBin == '2. 1080-2160 días') &
...
dfScore_mar23 <- read.csv('D:/!bso/vipCartera/scores/vipList_wScores_Abr2023.csv') %>% 
  select(CTACLIENTE, prob_GC, goodClient)

bdcAgoExp_mar23 <- bdcAgo_mar23 %>%
  ungroup() %>% 
...
vipListExp_mar23 <- dfScore_mar23 %>% 
  left_join(bdcVIPScore_exp_mar23, by = 'CTACLIENTE') %>% 
...
#==============================================================================
# final list
vipList_mar23 <- read.csv('D:/!bso/vipRC/vipListRC_Final_mar2023.csv')
#vipList <- vipListExp
table(vipList_mar23$target)
...
# #-----------------------------------------------------------------------------
# Final descriptives
vipDescFull_1_mar23 <- bdcAgo_mar23 %>% 
  left_join(vipList_feb23, by = 'CTACLIENTE') %>%
  ungroup() %>% 
  group_by(CI) %>% 
  mutate(ncli = ifelse(row_number()==1,1,0)) %>% 
...
length(unique(vipDescFull_1_mar23$CTACLIENTE)) 
# Operaciones que mantienen los VIPs activos al último cierre
nrow(vipDescFull_1_mar23)
  
vipDescFull_2_mar23 <- vipDescFull_1_mar23 %>% 
...
nrow(vipDescFull_2_mar23)

# Operaciones viables (>1,000$us)
nrow(vipDescFull_2_feb23[vipDescFull_2_feb23$Grupo_Amortización != '1. 20%-', ])
...
dailyMMcl_mar23 <- readRDS('C:/!bso/vipCartera/dailyMM_mar2023.rds')

vipDescFull_cc_mar23 <- readRDS('D:/!bso/vipRC/vipDescriptives_CompleteList_Mar2023_allVars.rds')
...
#===============================================================================
# infocred
infoRaw_mar23 <- fread('C:/!bso/Cargamensual_Infocred/BSO202302_utf8.txt', encoding = 'UTF-8')
nrowInfo <- nrow(infoRaw_mar23)

infoPerf_Compra_mar23 <- infoRaw_mar23 %>% 
  mutate(CI = paste0(`NRO DOCUMENTO`, EXT)) %>% 
  mutate(opBSO_ = ifelse(`SIGLA SBEF` == 'BSO', NumeroOp, '-')) %>% 
  mutate(dBSO_ = ifelse(`SIGLA SBEF` == 'BSO' & str_detect(`TIPO OBLIGADO SBEF`, 'A -'), 1, 0)) %>%
  mutate(isBSO_ = ifelse(`SIGLA SBEF` == 'BSO', 1, 0)) %>%
...
sum(cc_mar23$Saldo_USD_SF)
table(cc_mar23$Lista_GNN)
table(cc_mar23$Calificacion_SF)
...
check_feb23 <- vipLead_final_feb23 %>% 
  ungroup() %>% 
  group_by(CI) %>% 
  arrange(desc(CI)) %>% 
...
#==============================================================================
# Comparing pre-approveds

feb_sent <- read.xlsx('D:/!bso/vipCartera/LeadsCompraPreAprobados_Feb23_sent.xlsx',
                      sheet = 'aprobViables') %>% 
  select(CTACLIENTE, OPERACION, Oportunidad_Viable)
length(unique(feb_sent$CTACLIENTE))

feb_ae <- read.xlsx('D:/!bso/compraCartera/AllLeads_final_feb2023_ae_v2.xlsx')
feb_ae_viable <- feb_ae %>% 
  dplyr::filter(VIP_Viable == 'Viable') %>% 
  select(CTACLIENTE, OPERACION, Oportunidad_Viable)
feb_ae_nf <- feb_ae_viable %>% 
  anti_join(feb_sent, by = c('CTACLIENTE', 'OPERACION'))
length(unique(feb_ae_nf$CTACLIENTE))

feb_ae_nf2 <- feb_sent %>% 
  anti_join(feb_ae_viable, by = c('CTACLIENTE', 'OPERACION'))
length(unique(feb_ae_nf2$CTACLIENTE))

feb_ae_found <- feb_ae_viable %>% 
  full_join(feb_sent, by = c('CTACLIENTE', 'OPERACION')) %>% 
  mutate(dif = Oportunidad_Viable.x - Oportunidad_Viable.y)

bdcVipNoMiss_RB <- readRDS('D:/!bso/vipCartera/bdcDicVipNoMiss_feb2023_vRB.rds')
dataScore_feb23_RB <- fread('D:/!bso/vipCartera/dataScore_feb2023_vRB.csv')
clfTab_feb23_RB <- fread('D:/!bso/vipCartera/clfTab_feb2023_vRB.csv')
vipList_wScores_feb2023_vRB <- read.xlsx('D:/!bso/vipCartera/vipList_wScores_feb2023_vRB.xlsx')
vipListRC_Final_feb2023_vRB <- fread('D:/!bso/vipCartera/vipListRC_Final_feb2023_vRB.csv')
