...
write_rds(bdcFull, 'C:/!bso/vipCartera/output/bdcFull_ene20223.rds')
#===============================================================================
# Primeros filtros
bdcFull <- readRDS('//VFSNALSRV/RiesgoCrediticioOFN/Bases_Riesgos/MIS_SGNRFAR/analitica/vipCartera/output/bdcFull_ene2023.rds')
bdcVIP <- bdcFull %>% 
...
write_rds(bdcVIP, '//VFSNALSRV/RiesgoCrediticioOFN/Bases_Riesgos/MIS_SGNRFAR/analitica/vipCartera/output/bdcVIP_ene2023.rds')
bdcVIP <- readRDS('//VFSNALSRV/RiesgoCrediticioOFN/Bases_Riesgos/MIS_SGNRFAR/analitica/vipCartera/output/bdcVIP_ene2023.rds')
#===============================================================================
# bdc noviembre
bdcLast <- fread('C:/!bso/bases/Bases_Cartera/BaseCarteraEne2023.txt', 
...
bdcNovVip <- bdcVIP %>% 
  left_join(bdcNovCli, by = c('CI'))
bdcNovVipNoMiss <- bdcNovVip %>% 
  dplyr::filter(!is.na(moraMaxBin))

write_rds(bdcNovVipNoMiss, 'C:/!bso/vipCartera/output/bdcDicVipNoMiss_ene2023.rds')
write_rds(bdcNov, 'C:/!bso/vipCartera/output/bdcNov_ene2023.rds')
bdcNovVipNoMiss <- readRDS('C:/!bso/vipCartera/output/bdcDicVipNoMiss_ene2023.rds')
bdcNov <- readRDS('C:/!bso/vipCartera/output/bdcNov_ene2023.rds')
#===============================================================================
# infocred
infoRaw <- fread('C:/!bso/bases/utf/BSO202212_utf8.txt', encoding = 'UTF-8')
...
bdcNovInfo <- bdcNovVipNoMiss %>% 
  left_join(infoPerf, by = 'CI')


bdcAgoInfoCL <- bdcNovInfo
...
dailyMM_nov <- readRDS('C:/!bso/vipCartera/output/dailyMMcl_dic2022.rds') %>%
  glimpse() %>% 
  ungroup() %>% 
  select(-OPERACION, -maxMoraIM_op) %>% 
  group_by(CI, CTACLIENTE) %>% 
...
bus <- read.csv('C:/!bso/vipCartera/lista_clientes/Clientes excelentes Diciembre 2022.csv', sep='|') %>% 
...
write.xlsx(finalVIPdf, 'C:/!bso/vipCartera/output/finalVIPdf_histStr2_ene2023.xlsx')
#===============================================================================
# data for score selection
bdcFull<-readRDS('C:/!bso/vipCartera/output/bdcFull_ene2023.rds')
bdcVIPscore <- bdcFull %>% 
  select(OPERACION, CTACLIENTE, CI, DIASMORA, monDate, montous, FDESEMBOLSO, GENERO) %>%
  mutate(fdes = dmy(FDESEMBOLSO)) %>% 
  dplyr::filter(monDate <= 'mar. 2020'| monDate >= 'ene. 2021') %>% 
...
write_rds(bdcVIPscore, 'C:/!bso/vipCartera/output/bdcVIPscore_ene2023.rds')
bdcVIPscore <- readRDS('C:/!bso/vipCartera/output/bdcVIPscore_ene2023.rds')
...
write.csv(bdcVIPScore_t, 'C:/!bso/vipCartera/output/dataScore_ene2023.csv', row.names = F)
################################################################################
...
dfScore <- read.csv('C:/!bso/vipCartera/output/vipList_wScores_ene2023.csv') %>% 
...
write.csv(vipListExp, 'C:/!bso/vipCartera/output/vipListRC_Final_ene2023.csv', row.names = F)
write.xlsx(vipListExp, 'C:/!bso/vipCartera/output/vipListRC_Final_ene2023.xlsx')

#==============================================================================
# final list
vipList <- read.csv('C:/!bso/vipCartera/output/vipListRC_Final_ene2023.csv')
#vipList <- vipListExp
# #-----------------------------------------------------------------------------
# Final descriptives
vipDescFull_1 <- bdcNov%>% 
  select(-GENERO) %>% 
...
write.xlsx(vipDescFull_2, 'C:/!bso/vipCartera/output/vipDescriptives_CompleteList_ene2023_allVars.xlsx')

#-------------------------------------------------------------------------------
#REPORTE EXCEL PARA CUADRAR MARKDOWN
# This is where we make the final quarto report
# First time only: Read excel file and work on quarto report
vipReport <- read_xlsx('C:/!bso/vipCartera/output/vipDescriptives_CompleteList_Ene2023_allVars.xlsx') %>% 
 select(Lista_GNN, Monto_USD, Saldo_USD, Tipo_Crédito, Oportunidad_Viable, 
        VIP_Viable, Sucursal, NOMBRE_ASESOR, CI, Rango_Monto, AGENCIA,
        OPERACION, NOMBRE_TIT, PATERNO_TIT, MATERNO_TIT, Interés_USD, Probabilidad_VIP,
        Score, Decil_Score, ) %>% 
  dplyr::filter(VIP_Viable == 'Viable') %>% 
  mutate(Rango_Monto= case_when(Rango_Monto=='3. 1k-5k'~'1. 1k-5k',
                                Rango_Monto=='4. 5k-10k'~'2. 5k-10k',
                                Rango_Monto=='5. 10k-15k'~'3. 10k-15k',
                                Rango_Monto=='6. 15k-20k'~'4. 15k-20k',
                                Rango_Monto=='7. >20k'~'5. Mayor a 20k')) %>% 
  ungroup() %>% 
  group_by(Tipo_Crédito, Lista_GNN, VIP_Viable, Sucursal, AGENCIA) %>%
  mutate(TPPA = Interés_USD/Saldo_USD*100) 
  
agen <- read.csv('C:/!bso/girCartera/codigos_agencia.csv') %>% 
  dplyr::rename(agencia_name = AGENCIA,
                AGENCIA = COD_AGENCIA)   
vipReport_1 <- vipReport %>% 
  left_join(agen,by="AGENCIA") %>%
  select(-Interés_USD, -Score, -VIP_Viable) %>% 
  glimpse()

write.xlsx(vipReport_1, 'C:/!bso/vipCartera/output/reporteExcelDic22.xlsx')
################################################################################
#variacion mensual
...
################################################################################
# #Reporte excel solo para clientes en LISTA
# reporteLista <- read_xlsx('C:/!bso/vipCartera/output/vipDescriptives_CompleteList_Dic2022_allVars.xlsx') %>% 
#   select(Lista_GNN, Monto_USD, Saldo_USD, Tipo_Crédito, Interés_USD, 
#          Oportunidad_Viable, VIP_Viable, VIP_RC, Sucursal, NOMBRE_ASESOR, 
#          CI, Rango_Monto) %>% 
#   dplyr::filter(VIP_Viable == 'Viable',
#                 Lista_GNN == 'En Lista') %>% 
...
#                                 Rango_Monto=='7. >20k'~'5. Mayor a 20k')) %>% 
#   ungroup() %>% 
#   group_by(Tipo_Crédito, Lista_GNN, VIP_Viable, Sucursal, Rango_Monto) %>%
...
#   #mutate(total=sum(categ))
#   mutate(TPPA = Interés_USD/Saldo_USD*100) %>%
#   adorn_totals('row') %>% 
#   glimpse()
# 
# write.xlsx(reporteLista, 'C:/!bso/vipCartera/output/reporteExLista.xlsx')
################################################################################
...
dfCancel<-readRDS('C:/!bso/vipCartera/dfCancelEne15Ene23.rds') %>%  #Me la pasa el Gabo
mutate(montous = ifelse(MONEDA == 0, as.numeric(MONTO)/6.86, as.numeric(MONTO))) 
  
vipMiss<-vipList %>% 
  dplyr::filter(target==1) %>% 
  dplyr::filter(!(CI %in% bdcNov$CI)) %>% 
  left_join(dfCancel, by=c('CI', 'CTACLIENTE')) %>% 
  group_by(CI) %>% 
  arrange(CI,fdes) %>% 
  ...
  mutate(lastMonto=lastMonto/1000000, 
         totalMonto=totalMonto/1000000)
tab<-vipMiss %>% 
  ungroup() %>% 
...
write_rds(tab, 'C:/!bso/vipCartera/output/vipCancelados.rds')
