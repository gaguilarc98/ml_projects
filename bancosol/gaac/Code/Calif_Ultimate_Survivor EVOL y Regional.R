...
fechaajuste <- readRDS("D:/SCRIPTS R/Cosechas/Clientes_Ene15Ago23.rds") %>% 
  select(CTACLIENTE, OPERACION, FDES_ORI = fdes)

for(i in 1:length(mybdc)){
  tryCatch({
    print(mybdc[i])
    bdc <- readRDS(paste0('//VFSNALSRV/Bases_Riesgos/MIS_SGNRFAR/analitica/rdsGAR/ec_',mybdc[i],'.rds')) %>%
      filter(ctaCont %in% c('131','133','134','135','136','137','623')) %>% 
      ...
      mutate(pery = ifelse(year(FDES_ORI) < 2017, '< 2017', as.character(year(FDES_ORI)))) %>% 
      ...
      select(Fecha,CTACLIENTE,OPERACION,CI,GENERO,ESTADO, MODULO, NOMBRE_MODULO, Sucursal, DIASMORA,
             MONTOUS, previus, saldous, saldoMora, saldoCast, CONTINGENTE, SEC_PROD,
             MONEDA, TIPO_CRED = TIPO_CREDITO, tipoCred, FDES_ORI, pery)
    ...
    infoClean <- infoCheck %>% 
      ...
      mutate(across(califPeorSF, ~ max(.x,na.rm=T))) %>% #Para repetir los valores en cada fila
      ...
      select(CTACLIENTE, OPERACION, califPeorSF, CALIFICACION, saldoBSO)
