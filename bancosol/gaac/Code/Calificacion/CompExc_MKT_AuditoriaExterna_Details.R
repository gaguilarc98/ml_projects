...
sucFull <- read_xlsx("D:/!bso/califClientes/Clientes_Compartidos_MKT_Jul2023_xRegional.xlsx")  
#Para el infoJoin hacer correr el loop anterior para el mes correspondiente solo para 
#el mes que se debe actualizar
Comp <- infoJoin %>% 
  group_by(FECHA,  CI) %>% 
  arrange(desc(FDESEMBOLSO)) %>% 
  mutate(CTACLIENTE = CTACLIENTE[row_number()==1]) %>% 
...
sucFull <- sucFull %>% 
  bind_rows(compSuc) 
write_xlsx(sucFull, "D:/!bso/califClientes/Clientes_Compartidos_MKT_Jul2023_xRegional.xlsx")  
...
#En jul. 2023 el CI 4373922LP solo aparece 1 vez como BSO en infoCred, pese a que tiene
#Dos créditos en el cierre de ese mes solo que uno con CI 4373922
Comp <- infoJoin %>% 
  group_by(FECHA,  CI) %>% 
  arrange(desc(FDESEMBOLSO)) %>% 
  mutate(CTACLIENTE = CTACLIENTE[row_number()==1]) %>% 
...
write.xlsx(lista,paste0('D:/!bso/califClientes/Compartidos_Exclusivos_',mybdc,'.xlsx'))

#CHECK DE INTERGRIDAD ENTRE AMBOS ARCHIVOS
n_distinct(Comp$CI)
sum(sucFull$CLIENTES_COMP[sucFull$FECHA==as.Date("2023-07-31")])
...
y <- Comp %>% ungroup() %>%select(CI) %>%  distinct_all()
z <- Exc %>% ungroup() %>% select(CI) %>% distinct_all() %>% bind_rows(y) %>% 
  mutate(NDOC = str_replace(CI, "LP$|OR$|PO$|CB$|CH$|TJ$|SC$|BE$|PA$",""))
z$NDOC[!z$NDOC %in% x$NDOC]
####
...
write_xlsx(infoFullJoin, "D:/!bso/requests/BD_CARTERA_INFO_CALAgo2023.xlsx")
