...
for(i in 1:length(mybdc)){
  tryCatch({
    ...
    saveRDS(infoCheck,paste0('//VFSNALSRV/RiesgoCrediticioOFN/Bases_Riesgos/MIS_SGNRFAR/analitica/califClientes/process/comp_',mybdc[i],'.rds'))
...
mybdc <- c("Dic2022","Jun2023","Jul2023")
mybdc <- c("Ago2023")
infoList <- list()
  ...
  tryCatch({
    ...
    bdcBSO_full <- readRDS(paste0('D:/!bso/girCartera/rds/ec_',mybdc[i],'.rds')) %>%
      ...
      select(Fecha,CTACLIENTE,OPERACION,CI,GENERO,ESTADO, MODULO, Sucursal, DIASMORA,
             ...
             MONEDA, TIPO_CRED = TIPO_CREDITO, tipoCred, fdes, esFSL)
...
####____EXCEL____####
comp <- read_xlsx("D:/!bso/shared/ops_ClientesCompFSL_MayJun2023.xlsx")
compMay <- comp %>% dplyr::filter(Fecha==as.Date("2023-05-31"))

zz <- compMay %>% dplyr::filter(!(CI %in% x$CI_info))
ww <- x %>% dplyr::filter(!(CI_info %in% compMay$CI))
...
infoPerf %>% #Check de cuadre con meses anteriores
  group_by(Fecha) %>% 
...
write_xlsx(Informe, paste0("D:/!bso/califClientes/output/lastComp_",shortmonth,".xlsx"))
...
####____CUADRE DE CLIENTES COMPARTIDOS____####
ddbb <- read_xlsx("D:/!bso/califClientes/DDBB_Compartido.xlsx", sheet = "DDBB")
ddbbComp <- ddbb %>% 
  dplyr::filter(Fecha==as.Date("2023-03-31")) %>% 
  dplyr::filter(ZSIGLA_B=="BSO") 

y <- ddbbComp %>% 
  group_by(ZSistema) %>% 
  summarise(S=sum(CBRUTA_USD))
  
my <- c("Mar2023","Abr2023","May2023","Jun2023","Jul2023")

sigla <- list()
for (i in 1:length(my)) {
  print(my[i])
  infocred <- readRDS(paste0("D:/!bso/califClientes/process/comp_",my[i],".rds"))
  infoSIGLA2 <- infocred %>% 
    select(ENTIDAD, SIGLA) %>% 
    distinct_all()
  sigla[[i]] <- infoSIGLA2
}

siglaFull <- rbindlist(sigla) %>% 
  distinct_all()
write_xlsx(siglaFull, "D:/!bso/califClientes/Siglas_del_Orto.xlsx")
####____LOOP CUADRE____
siglas <- read_xlsx("D:/!bso/califClientes/Siglas_del_Orto.xlsx") %>% 
  select(-ENTIDAD, SIGLA, SiglaClean=Sigla, EntidadClean = Entidad) %>% 
  distinct_all()

mon <- c("Mar","Abr","May","Jun","Jul")
year <- c(2023)
myDate <- as.vector(sapply(year, function(x){paste0(mon,". ",year)}))
my <- as.vector(sapply(year, function(x){paste0(mon,year)}))
i <- 1
infoList <- list()
for (i in 1:length(my)) {
  print(my[i])
  infocred <- readRDS(paste0("D:/!bso/califClientes/process/comp_",my[i],".rds"))
  infoBSO <- infocred %>% 
    mutate(monDate = as.yearmon(myDate[i])) %>% 
    dplyr::filter(str_detect(TIPO_OBLIGADO, "A -")) %>% 
    mutate(esDeudorBSO = ifelse(SIGLA=="BSO" & str_detect(TIPO_OBLIGADO, "A -"), 1, 0)) %>% 
    mutate(esDeudorSF = ifelse(SIGLA!="BSO" & str_detect(TIPO_OBLIGADO, "A -"), 1, 0)) %>% 
    dplyr::filter(!(esDeudorSF==1 & REGULADO!="SBEF")) %>% 
    left_join(siglas, by="SIGLA") %>% 
    dplyr::filter(!is.na(SiglaClean)) %>%
    select(monDate, esDeudorBSO, esDeudorSF, SiglaClean, EntidadClean, 
           saldo, saldoVig, saldoMora, saldoCast, saldoCont) %>% 
    group_by(monDate, esDeudorBSO, esDeudorSF, SiglaClean, EntidadClean) %>% 
...
infoFull <- rbindlist(infoList) %>% 
  mutate(monDate = as.Date(monDate, frac=1))
write_xlsx(infoFull, "D:/!bso/califClientes/CuadreASOFIN.xlsx")

infoFull %>% group_by(monDate, esDeudorBSO) %>% 
  summarise(Saldo=sum(saldo),SaldoMora=sum(saldoMora)) %>% 
  pivot_wider(names_from = esDeudorBSO, values_from = c(Saldo, SaldoMora))

#Compartidos saldo BSO
infoBSO <- infocred %>% 
  dplyr::filter(SIGLA=="BSO" & str_detect(TIPO_OBLIGADO, "A -")) 
sum(infoBSO$saldo)
sum(infoBSO$saldoMora)

infoBSO <- infocred %>% 
  mutate(esDBSO = ifelse(SIGLA=="BSO" & str_detect(TIPO_OBLIGADO, "A -"),1,0)) %>% 
  # group_by(CI) %>% 
  # dplyr::filter(max(esDBSO)==1) %>% 
  # ungroup() %>% 
  dplyr::filter(SIGLA!="BSO" & str_detect(TIPO_OBLIGADO, "A -")) %>%
  dplyr::filter(REGULADO=="SBEF") %>% 
  mutate(saldoPar0 = ifelse(DiasMora>0, saldo, 0)) %>% 
  mutate(sig = substr(SIGLA,1,3)) %>% 
  left_join(siglas, by="SIGLA") %>% 
  dplyr::filter(!is.na(SiglaClean))
  dplyr::filter(str_detect(ENTIDAD,"BANCO|CIDRE|COOPERATIVA|CRECER|DIACONIA|PROGRESO|FONDECO|FUBODE|PRO MUJER|IDEPRO|IMPRO|PRIMERA ENTIDAD|PROMOTORA|SARTAWI"))

  x <- infoBSO %>% 
    mutate(SaldoCB_Cont = ifelse(saldoCont>0 & saldoCont<saldo, saldo-saldoCont,0)) %>% 
    group_by(SIGLA) %>% 
    summarise(S=sum(saldo), Sp0=sum(saldoPar0), SM=sum(saldoMora), SC = sum(saldoCont), 
              SCa=sum(saldoCast), SCont = sum(SaldoCB_Cont)) %>% 
    adorn_totals("row")
sum(infoBSO$saldo)
sum(infoBSO$saldoMora)


####____DEUDORES Y CODEUDORES COMPARTIDOS____####
y <- infoCheck %>% 
  mutate(esDeudor = ifelse(str_detect(TIPO_OBLIGADO, "A - "),1,0)) %>% 
  mutate(esCodeudor = ifelse(str_detect(TIPO_OBLIGADO, "B - "),1,0)) %>% 
  group_by(SIGLA, NOP_ORIGINAL) %>% 
  mutate(Deudor = max(esDeudor)) %>% 
  mutate(Codeudor = max(esCodeudor)) %>% 
  mutate(TieneAmbos  = ifelse(Deudor==1 & Codeudor==1, 1, 0)) %>% 
  ungroup() %>% 
  dplyr::filter(REGULADO=="SBEF") %>% 
  dplyr::filter(str_detect(TIPO_OBLIGADO, 'A - ')) %>% 
  mutate(esBSO=ifelse(SIGLA=='BSO',1,0)) %>%
  ...
  mutate(DeudorCod = max(TieneAmbos[esBSO == 0])) %>%
  ungroup() %>% 
  dplyr::filter(califPeorSF!="_" & SIGLA=="BSO") %>% 
  select(CTACLIENTE, OPERACION, CI, califPeorSF, ESTADOPeorSF, MaxDiasMoraSF,DeudorCod)

z <- y %>% 
  dplyr::filter(califPeorSF=="F" & ESTADOPeorSF =="1. VIGENTE" & DeudorCod==1)
