...
for(i in 1:length(short_list)){
  ...
  lim<-bdcLim %>% 
    select(monDate, saldous, AGENCIA, saldoReprog) %>%
    mutate(reprog=ifelse(saldoReprog== saldous, 'Reprog', 'No reprog')) %>% 
    group_by(monDate, reprog) %>% 
...
write_rds(base, 'C:/!bso/concentracion/output/baseLimites_reprog.rds')
base<-readRDS('C:/!bso/concentracion/output/baseLimites_destino.rds')
################################################################################
#Tabla de Estadísticos
tabla_resumen<-base %>% 
  ungroup() %>% 
  dplyr::filter(reprog!= is.na(reprog)) %>% 
  select(reprog, monDate, carteraBso, pct_saldo) %>% 
  group_by(reprog) %>% 
...
#Pronósticos
baseFcast<-base %>% 
  dplyr::filter(!is.na(reprog)) %>% 
  dplyr::filter(monDate>'feb. 2015') %>% 
  select(monDate, reprog, carteraBso) %>% 
  dplyr::rename(saldo=carteraBso) %>% 
  mutate(reprog=str_replace(reprog, ' ', '_')) %>%
  group_by(reprog) %>% 
  arrange(monDate) %>% 
  ...
  pivot_longer(!c(monDate, reprog)) %>% 
  arrange(reprog, monDate)

for(i in 1:length(unique(baseFcast$reprog))) {
  for(k in 1:length(unique(baseFcast$name))) {
    tryCatch({
      print(paste0(i, ', ', k))
      
      gph <- baseFcast %>% 
        dplyr::filter(reprog == unique(baseFcast$reprog)[i] & name == unique(baseFcast$name)[k]) %>% 
        ungroup()
      ...
      exp <- gph %>%
        ...
        mutate(D = fit$arma[7]) %>%
        mutate(Q = fit$arma[4]) %>%
        fill(reprog, name, .direction = 'updown') %>% 
        relocate(date) %>% 
        mutate(monDate = as.yearmon(date))
      
      write_rds(exp, paste0("C:/!bso/concentracion/output/rdsFCST_m_reprog/m_", unique(baseFcast$reprog)[i], '_',
...
################################################################################
lim<-read.xlsx('C:/!bso/concentracion/limitesreprog.xlsx')
gc()
cc_list <- list.files('C:/!bso/concentracion/output/rdsFCST_m_reprog')
cc_list_2<-list()

for(i in 1:length(cc_list)){
  df<-readRDS(paste0('C:/!bso/concentracion/output/rdsFCST_m_reprog/',
                     cc_list[[i]]))
  cc_list_2[[i]]<-df
}
################################################################################
...
allExp_2<-bind_rows(cc_list_2) %>% 
  ...
  select(monDate, reprog, value, name, starts_with('Hi ') ) %>%
  dplyr::filter(name=='saldo') %>% 
  dplyr::filter(month(monDate)> 3 & year(monDate)==2023) %>% 
  left_join(lim, by='reprog') %>% 
  select(-name) %>% 
  ...
  pivot_longer(!c(monDate, reprog, LimiteInicial, LimiteFinal)) %>% 
  arrange(monDate, name, reprog) %>% 
  group_by(monDate, name) %>% 
  mutate(carteraTotal=sum(value)) %>% 
  ungroup() %>% 
  group_by(monDate, reprog) %>% 
  arrange(monDate, reprog, desc(name)) %>% 
  ...
  arrange(reprog, monDate, desc(name))

write_rds(allExp_2, 'C:/!bso/concentracion/proyreprog.rds')
allExp_2 <- readRDS('C:/!bso/concentracion/proyreprog.rds')

for(i in 1:length(unique(allExp_2$reprog))) {
  
  all_gph <- bind_rows(cc_list_2) %>% 
    # select(monDate, rangom, value, name, `Hi 99`) %>% 
    dplyr::filter(name=='saldo') %>% 
    left_join(lim, by='reprog') %>% 
    dplyr::filter(reprog==unique(allExp_2$reprog)[[i]])
  ...
  gph <- all_gph %>% 
    select(reprog, monDate, value,  `Lo 99`,  `Lo 95`,  `Hi 99`,  `Hi 95`) %>% 
  ...
  ggsave(paste0('C:/!bso/concentracion/png/reprog_',unique(allExp_2$reprog)[[i]],
                '.png'))
...
for(i in 1:length(unique(allExp_2$reprog))) {
  ...
  gph <- all_gph %>% 
    select(reprog, monDate, value,  `Lo 99`,  `Lo 95`,  `Hi 99`, 
  ...
  ggsave(paste0('C:/!bso/concentracion/png/reprog_pct_',unique(allExp_2$reprog)[[i]],
                '.png'))
  
}
################################################################################
#exportar ppt
allexp_ppt <- readRDS('C:/!bso/concentracion/proyreprog.rds') %>%  
  select(monDate, reprog, LimiteInicial, LimiteFinal, pct, value, total_Flag_Inicial,
         total_Flag_Final, first_flag_inicial) %>%
  #dplyr::filter(monDate=='dic. 2023') %>% 
  dplyr::filter(first_flag_inicial=='TRUE')

write.xlsx(allexp_ppt, 'C:/!bso/concentracion/output/tablaReprog.xlsx')
