...
for(i in 1:(length(myrds)-lag)) {
  tryCatch({
    ...
    df1 <- readRDS(paste0('D:/!bso/girCartera/rdsGAR/ec_', myrds[i], '.rds')) #se abre mes anterior
    
    df2 <- readRDS(paste0('D:/!bso/girCartera/rdsGAR/ec_', myrds[k], '.rds')) #se abre mes posterior
    ...
    df3 <- df1[!(df1$OPERACION %in% df2$OPERACION),] %>% 
      ...
      select(OPERACION, CI, CTACLIENTE, FDESEMBOLSO,fdes, MONTO, MONEDA, saldous, CALIFICACION, ESTADO,
...
write_rds(dfCancel,"D:/!bso/Consultas/dfCancelEne15Ene23.rds") #Se guarda la base de cancelados
dfCancel <- readRDS("D:/!bso/Consultas/dfCancelEne15Ene23.rds")
print(dfCancel %>% 
...
bdcCancel <- readRDS('D:/!bso/transMat/matCancel.rds')
myrds <- c('Feb2023','Mar2023') #Colocar nombre MonthYYYY del mes previo y mes actual
tryCatch({
  print(myrds[1])
  print(myrds[2])
...
write_rds(dfCancel,"D:/!bso/Consultas/dfCancelEne15Mar23.rds") #change name of file
...
for (i in 1:length(myrds)) {
  ...
  bdc <- readRDS(paste0('D:/!bso/girCartera/rdsGAR/ec_', myrds[i], '.rds')) %>% 
    ...
    select(-saldoCast)
  #Se almacena cada base de operacion, califiación y otros en una lista
  bdcList[[i]] <- bdc
}
gc()
...
myrds <- 'Mar2023' #Colocar MonthYYYY del mes actual
print(myrds)
...
tm_ops <- bdcTrans %>% 
  ...
  ungroup() %>%
  bind_rows(bdcCancel) %>% #Aquí se añade la tabla resumen de cancelados
...
write.csv(tm_ops,"D:/!bso/transMat/Oreports/tmAll_Mar23.csv",row.names = F)
...
tm_ops <- read.csv("D:/!bso/transMat/Oreports/tmAll_Mar23.csv") %>% 
  mutate(monDate=as.yearmon(monDate))

tm2022_prob <- tm_ops %>% 
  dplyr::filter(monDate > 'mar. 2022') %>% #Se filtra los meses que se mostrarán en el reporte
...
tm2022_ops <- tm_ops %>% 
  dplyr::filter(as.yearmon(monDate) > 'mar. 2022') %>% #Designación de temporalidad 
  ...
  relocate(monDate,cm1,A,B,C,D,E,`F`,S,Z)
tm2022_saldo <- tm_ops %>% 
  dplyr::filter(as.yearmon(monDate) > 'mar. 2022') %>% #Se delimita temporalidad
  mutate(monDate=as.yearmon(monDate)) %>% 
  arrange(monDate) %>% 
...
tm2022_difPrev <- tm_ops %>% 
  dplyr::filter(monDate > 'mar. 2022') %>% #Se delimita temporalidad
  arrange(monDate) %>% 
...
write.xlsx(MatTrans,'D:/!bso/transMat/tm_Mar2023.xlsx')
