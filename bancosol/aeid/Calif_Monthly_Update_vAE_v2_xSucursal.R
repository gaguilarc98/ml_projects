...
for(i in 1:length(myutf)){
  tryCatch({
    ...
    bdcBSO_full <- readRDS(paste0('D:/!bso/girCartera/rds_v3/',
      ...
      select(CTACLIENTE,OPERACION,CI, previus, saldous, saldoMora,
             DIASMORA, GENERO,ESTADO,MODULO,Fecha, sucursal) %>%
...
write_rds(infoFull,'C:/!bso/califClientes/infoFull_xSuc_feb23.rds')
write_rds(bsoFull,'C:/!bso/califClientes/bsoFull_xSuc_feb23.rds')
infoList <- NULL
bsoList <- NULL
infoFull <- readRDS('C:/!bso/califClientes/infoFull_xSuc_feb23.rds')
bsoFull <- readRDS('C:/!bso/califClientes/bsoFull_xSuc_feb23.rds')
...
infoPerf_SUC <- infoFull %>% 
  # dplyr::filter(MODULO!=131) %>%
  # dplyr::filter(ESTADO!="CASTIGADA") %>%
  group_by(Fecha,CI,GENERO,entidadSF_2, CTACLIENTE, OPERACION, sucursal) %>% 
...
infoPerf_SUC <- infoFull %>% 
  ...
  summarise(saldo=sum(saldous),saldoMora=sum(saldoMora),
            ...
            saldoMMCNoBSO=max(saldoMMCNoBSO)) %>% 
  ungroup() %>% 
  left_join(bsoFull,by=c("Fecha", 'sucursal')) %>%
  mutate(Fecha=as.Date(Fecha)) %>% 
  mutate(Cliente = 1) %>% 
  mutate(PeorCalif=ifelse(califBSO_2<califSF_2,1,0)) %>% 
  mutate(MejorCalif=ifelse(califBSO_2>califSF_2,1,0)) %>% 
...
write_rds(infoPerf_SUC,'C:/!bso/califClientes/infoPerf_xSuc_feb23.rds')
infoPerf_SUC <- readRDS('C:/!bso/califClientes/infoPerf_xSuc_feb23.rds')
...
table_1_suc <- infoPerf_SUC %>% 
  ungroup() %>% 
  group_by(CTACLIENTE, Fecha) %>% 
  arrange(CTACLIENTE, desc(Fecha)) %>% 
  mutate(saldoNoBSO = ifelse(row_number() > 1, 0, saldoNoBSO)) %>% 
  ungroup() %>% 
  group_by(Fecha, sucursal) %>% 
