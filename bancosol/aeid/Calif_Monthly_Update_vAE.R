...
for(i in 1:length(myutf)){
  tryCatch({
    ...
    infoCleanA <- infoRaw %>%
      ...
      mutate(HISTORICO = as.character(round(HISTORICO)))
...
gph <- table_1 %>% 
  pivot_longer(!Fecha) %>% 
  dplyr::filter(name == 'pct_SaldoComp' | name == 'pct_ClientesComp') %>% 
  group_by(name) %>% 
  arrange(name, Fecha) %>% 
  mutate(d1 = value - dplyr::lag(value, 12))

ggplot(gph, aes(x = Fecha, y = value, color = name)) + 
  geom_line(linewidth = 1.15) +
  scale_y_continuous(limits = c(20, 35))

ggplot(gph, aes(x = Fecha, y = d1, color = name)) + 
  geom_line(linewidth = 1.15)
################################################################################
