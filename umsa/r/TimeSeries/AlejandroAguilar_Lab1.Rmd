...
d <- decompose(dat.ts)
plot(dat.ts,type='l',pch=20,xlab = "Fecha",ylab = "Temperatura mínima [K]")
lines(d$trend,lwd=2,col="red")
```
...
d <- decompose(dat.ts)
plot(dat.ts,type='l',pch=20,xlab="Fecha",ylab="Producción de electricidad (Millones de kWh)")
lines(d$trend,lwd=2,col="red")
```
...
lines(media_s,col="blue")
#DESCOMPOSICION DE LA SERIE TEMPORAL
dat.ts <- ts(dat$ICE,start=decimal_date(as.Date(dat$Date[1])),frequency = 12)
d <- decompose(dat.ts)
